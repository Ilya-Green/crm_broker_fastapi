...
const render = {
  ...
  notes: function render(data, type, full, meta, fieldOptions) {
    ...
    return `<div class="d-flex flex-row">${data
      ...
      .map(
        (e) =>
          `<a class='mx-1 btn-link' href="${e._detail_url}"><span class='m-1 py-1 px-2 badge bg-blue-lt lead d-inline-block text-truncate' data-toggle="tooltip" data-placement="bottom" title='${e.content}'  style="max-width: 20em;">${e.content}</span></a>`
      )
      .join("")}</div>`;
  },
